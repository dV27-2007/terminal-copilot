...
term_copilot_bash_accept() {
  ...
  payload="$(TERM_COPILOT_BUFFER="$READLINE_LINE" TERM_COPILOT_CURSOR="$READLINE_POINT" TERM_COPILOT_CWD="$PWD" python3 - <<'PY' 2>/dev/null
import json, os
payload = {
...
PY
  ...
  response="$(TERM_COPILOT_PAYLOAD="$payload" TERM_COPILOT_ENDPOINT="/predict" python3 - <<'PY' 2>/dev/null
import os, urllib.request
url = os.environ.get("TERM_COPILOT_URL", "http://127.0.0.1:8765") + os.environ["TERM_COPILOT_ENDPOINT"]
req = urllib.request.Request(url, data=os.environ["TERM_COPILOT_PAYLOAD"].encode(), headers={"Content-Type":"application/json"})
try:
  print(urllib.request.urlopen(req, timeout=0.45).read().decode(), end="")
except Exception:
  pass
PY
)"
